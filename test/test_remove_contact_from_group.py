...
def test_remove_contact_from_group(app, orm):
    ...
    if len(contacts_in_group) == 0:
        if len(orm.get_contact_list()) == 0:
            app.contact.add_new(Contact(firstname="Ekaterina", middlename="Aleksandrovna", lastname="Pentjuhina",
                                        ...
                                        address2="address", phone2="home-phone", notes="notes", bday="14",
                                        bmonth="October",
    ...
    contacts_in_group_new = orm.get_contacts_in_group(group)
    contacts_in_group_ui = app.contact.get_contacts_in_group(group.id)
    assert sorted(contacts_in_group_new, key=Contact.id_or_max) == sorted(contacts_in_group_ui, key=Contact.id_or_max)
